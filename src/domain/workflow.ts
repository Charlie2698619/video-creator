...
export function getWorkflowBlockers(project: VideoProject): string[] {
  ...
  if (project.storyboard && !project.scenePlan) blockers.push('Scene plan is required before source generation.')
  if (project.scenePlan && !project.artifacts.sourceBundle) blockers.push('HyperFrames source is required before rendering.')
